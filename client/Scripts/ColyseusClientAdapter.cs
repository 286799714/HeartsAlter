...
public sealed class ColyseusClientAdapter
{
        ...
        string endpoint = DefaultEndpoint)
    {
        ...
        {
            _client = new Client(endpoint);
            // Seat reservations generated by a remote deployment may contain
            // an internal/publicAddress value (for example 127.0.0.1). The
            // user-selected endpoint is the reachable authority, so let the
            // SDK build the room WebSocket URL from this client instead.
            var seat = reservation.ToSeatReservation();
            seat.publicAddress = string.Empty;
            _room = await _client.ConsumeSeatReservation<MyRoomState>(
                seat, new Dictionary<string, string>());
            RegisterRoomHandlers(_room);
            await _room.WaitForFirstState();
            await _room.Send("request_hand");
