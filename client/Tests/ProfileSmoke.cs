...
public partial class ProfileSmoke : Node
{
	...
	{
		var lobby = new ColyseusLobbyAdapter { PlayerName = "Profile smoke" };
		...
		try
		{
			...
			lobby.PlayerName = "Must not replace saved name";
			...
			foreach (string scene in new[] { "PlayerInfo", "PlayerInfoHorizontal", "MainPlayerInfo" })
			{
				var widget = GD.Load<PackedScene>($"res://scenes/in_game/{scene}.tscn").Instantiate<PlayerInfo>();
				AddChild(widget);
			...
			GD.Print("PROFILE_SMOKE_OK: private save, reconnect, C# schema, reservation and all avatar scenes");
		}
