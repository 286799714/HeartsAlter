...
有领出花色时必须跟花色。房间提供两个独立开关，均默认开启：`heartsBreakingEnabled` 控制红心未破时禁止领出红桃（只剩红桃除外）；`mustDiscardPointsWhenVoid` 控制缺门时必须优先垫分牌（红桃、黑桃 Q），关闭后缺门可出任意手牌，首墩也适用。首墩领出或跟花色时避免分牌的限制独立保留，真人、机器人和超时代打遵循相同规则。

创建房间或大厅 `create_room` 可传入这两个布尔选项。准备阶段房主可发送 `set_rules`，例如 `{ "heartsBreakingEnabled": false }` 只关闭碎心规则；当前值通过同名公开状态字段同步。实际修改会取消其他真人的准备，房主与机器人保持准备；非房主、非布尔值及开局后的修改会收到 `invalid_play`。选项在下一局继续保留。
